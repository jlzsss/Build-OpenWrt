...
git clone --depth 1 --filter=blob:none --sparse https://github.com/coolsnowwolf/lede.git temp-lede && cd temp-lede && git sparse-checkout set pcre && cd /workdir/openwrt && mv /workdir/openwrt/temp-lede/package/libs/pcre package && rm -rf /workdir/openwrt/temp-lede
